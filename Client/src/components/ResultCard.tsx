...
export default function ResultCard({ risk, probability, explanation, role = 'patient', feature_impacts, payload }: ResultCardProps) {
  ...
  const riskTextColor = isHighRisk ? 'text-rose-600' : isModerateRisk ? 'text-amber-600' : 'text-[#17805d]';
  const riskBgPulse = isHighRisk ? 'bg-rose-500 shadow-[0_0_10px_#f43f5e]' : isModerateRisk ? 'bg-amber-500 shadow-[0_0_10px_#f59e0b]' : 'bg-[#17805d] shadow-[0_0_10px_#10b981]';
  const gaugeStroke = isHighRisk ? '#f43f5e' : isModerateRisk ? '#f59e0b' : '#17805d';
  
  const topBannerAccentBorder = isHighRisk 
    ? 'border-t-[4px] border-t-rose-400 border-x border-b border-slate-300/70' 
    : isModerateRisk 
    ...
    <div className="w-full text-slate-900 space-y-10 sm:space-y-14 lg:space-y-16">
          ...
          <div className="flex-1 self-stretch flex flex-col justify-between text-left py-1 sm:py-2">
            ...
            <div className="space-y-1.5 py-2 sm:py-3.5">
              <div className="flex flex-wrap items-baseline gap-2.5 sm:gap-3">
                <span className="text-xl sm:text-3xl md:text-4xl font-black text-slate-900 tracking-tight">Risk Estimate:</span>
                <span className={`text-4xl xs:text-5xl sm:text-6xl md:text-6.5xl font-black tracking-tight ${riskTextColor}`}>
              ...
              <div className="flex flex-wrap items-center gap-2 sm:gap-3 pt-0.5">
                ...
                <span className={`px-3 sm:px-3.5 py-1 rounded-full text-xs font-black shadow-sm uppercase tracking-wider ${
                  isHighRisk 
            ...
            {/* Bottom item */}
            <div className="pt-2 sm:pt-2.5 border-t border-slate-300/70 flex flex-wrap items-center gap-x-2.5 sm:gap-x-3.5 gap-y-1 text-[11px] sm:text-xs font-bold text-slate-600">
              <span className="flex items-center gap-1.5 text-blue-900 font-extrabold">
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                Model Confidence: High (95.8% Acc.)
              </span>
      ...
      <div className="p-4 sm:p-6 md:p-8 neu-flat rounded-3xl border border-slate-200/80 bg-gradient-to-b from-white/95 to-[#eef3f8]/90 shadow-[6px_6px_20px_rgba(163,177,198,0.35),-6px_-6px_20px_rgba(255,255,255,0.9)] space-y-4 sm:space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 sm:pb-4 border-b border-slate-200/60">
          <div>
            <h3 className="text-lg sm:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2 sm:gap-2.5">
              <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-blue-600 text-white flex items-center justify-center shadow-md shrink-0">
              ...
              <span>WHAT CONTRIBUTED MOST?</span>
            </h3>
        ...
        <div className="divide-y divide-slate-200/80 pt-1">
          {topDrivers.map((driver, index) => (
            <div 
              ...
              className={`py-3.5 sm:py-5 ${index === 0 ? 'pt-1' : ''} ${index === topDrivers.length - 1 ? 'pb-1' : ''} space-y-2.5 hover:bg-slate-50/60 px-1.5 sm:px-3 rounded-2xl transition-colors`}
            >
              <div className="flex flex-col xs:flex-row sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center gap-2.5 sm:gap-3">
                  {/* Raised Circular Neumorphic Button with Dot */}
                  <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full neu-flat flex items-center justify-center shadow-[2px_2px_5px_#c2d2e4,-2px_-2px_5px_#ffffff] shrink-0">
                    <div 
                      className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full shadow-sm"
                      style={{ backgroundColor: driver.dotColor }}
                    />
                  </div>
                  <span className="font-black text-xs sm:text-base text-slate-900">{driver.name}</span>
                  <span className={`font-bold text-[11px] sm:text-sm px-2 sm:px-2.5 py-0.5 rounded-lg border shadow-sm ${driver.metricBadgeClass}`}>
                    {driver.metric}
                  </span>
                </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 lg:gap-8 items-stretch">
        ...
        {/* Clean Audit Table */}
        <div className="xl:col-span-6 p-4 sm:p-6 md:p-8 neu-flat rounded-3xl border border-slate-200/80 bg-gradient-to-b from-white/95 to-[#eef3f8]/90 shadow-[6px_6px_20px_rgba(163,177,198,0.35),-6px_-6px_20px_rgba(255,255,255,0.9)] h-full flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-4 sm:mb-5 pb-3 border-b border-slate-200/60">
              <div>
                <h3 className="text-base sm:text-xl font-black text-slate-900 tracking-tight flex items-center gap-2">
                  <span className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-teal-700 text-white flex items-center justify-center shadow-sm shrink-0">
                    <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                  </span>
                  <span>CLINICAL DATA SUMMARY</span>
                </h3>
                <p className="text-[11px] sm:text-xs font-bold text-slate-500 mt-0.5">Audit record of submitted patient clinical parameters.</p>
              </div>
              <span className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 bg-slate-200/80 px-2 sm:px-2.5 py-1 rounded-full shrink-0">
                Submitted
              </span>
            </div>

            <div className="overflow-x-auto touch-scroll no-scrollbar -mx-2 sm:mx-0 px-2 sm:px-0">
        ...
        <div className="xl:col-span-6 p-4 sm:p-6 md:p-8 neu-flat rounded-3xl border border-rose-200/80 bg-gradient-to-b from-rose-50/30 via-white to-white shadow-[6px_6px_20px_rgba(163,177,198,0.35),-6px_-6px_20px_rgba(255,255,255,0.9)] h-full flex flex-col justify-between">
          <div>
            <div className="flex flex-wrap items-center justify-between gap-2.5 mb-4 sm:mb-5 pb-3 border-b border-rose-200/60">
              <div className="flex items-center gap-2.5 sm:gap-3">
                <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-rose-500 text-white flex items-center justify-center shadow-md shrink-0">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                </span>
                <div>
                  <h3 className="text-base sm:text-xl font-black text-rose-950 tracking-tight">FLAGGED FACTORS</h3>
                  <p className="text-[11px] sm:text-xs font-bold text-rose-700">Metrics outside normal clinical ceilings requiring follow-up</p>
                </div>
              </div>

              {flaggedFactors.length > 0 && (
                <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-wider px-2.5 sm:px-3 py-1 rounded-full bg-rose-100 text-rose-900 border border-rose-200 shadow-sm shrink-0">
            ...
            <div className="space-y-3">
              {flaggedFactors.length > 0 ? (
                flaggedFactors.map((item, idx) => (
                  <div 
                    key={idx} 
                    className="p-3.5 sm:p-4 rounded-2xl bg-white/95 border border-rose-200/90 shadow-sm hover:shadow-md transition-all space-y-2.5"
                  >
                    <div className="flex flex-wrap items-center justify-between gap-2 pb-2 border-b border-rose-100/80">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="w-2.5 h-2.5 rounded-full bg-rose-600 animate-pulse shrink-0" />
                        <span className="font-black text-xs sm:text-sm text-slate-900">{item.name}</span>
                        <span className="text-[11px] sm:text-xs font-black text-slate-800 bg-slate-100 px-2 sm:px-2.5 py-0.5 rounded-lg border border-slate-200/80">
                          Current: <strong className="text-rose-950">{item.value}</strong>
                        </span>
                      </div>
                      <span className={`text-[10px] sm:text-[11px] font-black uppercase tracking-wider px-2.5 sm:px-3 py-0.5 rounded-full shadow-sm shrink-0 ${
                        item.severity === 'high' 
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-baseline gap-1 sm:gap-2 text-xs text-slate-700 font-medium bg-rose-50/50 p-2.5 sm:p-3 rounded-xl border border-rose-100/80">
                ...
                <div className="p-6 sm:p-8 text-center neu-inset rounded-2xl text-emerald-800 font-black text-xs sm:text-sm flex items-center justify-center gap-2">
                  <CheckCircle2 className="w-5 h-5 text-emerald-600 shrink-0" />
                  <span>All submitted clinical parameters are within baseline reference limits.</span>
                </div>
              )}
            </div>
          </div>

          <div className="mt-4 sm:mt-5 pt-3 sm:pt-3.5 border-t border-rose-100 text-[10px] sm:text-[11px] text-slate-500 font-semibold italic">
      ...
      <div className="p-4 sm:p-6 md:p-8 neu-flat rounded-3xl border border-slate-200/80 bg-gradient-to-b from-white/95 to-[#eef3f8]/90 shadow-[6px_6px_20px_rgba(163,177,198,0.35),-6px_-6px_20px_rgba(255,255,255,0.9)]">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5 sm:mb-6 pb-3 sm:pb-4 border-b border-slate-200/60">
          <div>
            <h3 className="text-lg sm:text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2 sm:gap-2.5">
              <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-purple-600 text-white flex items-center justify-center shadow-md shrink-0">
          ...
          <span className="text-[10px] sm:text-[11px] font-black uppercase tracking-wider px-3 py-1 rounded-full bg-purple-100 text-purple-900 border border-purple-200/80 self-start sm:self-auto shrink-0">
            Clinical Prompts
          </span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5 sm:gap-4">
          <div className="p-4 sm:p-5 rounded-2xl neu-inset bg-gradient-to-r from-blue-50/40 via-white to-transparent border-l-4 border-blue-600 space-y-1.5">
          ...
          <div className="p-4 sm:p-5 rounded-2xl neu-inset bg-gradient-to-r from-amber-50/40 via-white to-transparent border-l-4 border-amber-500 space-y-1.5">
            <h4 className="font-black text-xs sm:text-sm text-slate-900 flex items-center gap-2">
          ...
          <div className="p-4 sm:p-5 rounded-2xl neu-inset bg-gradient-to-r from-red-50/40 via-white to-transparent border-l-4 border-red-700 space-y-1.5">
            <h4 className="font-black text-xs sm:text-sm text-slate-900 flex items-center gap-2">
          ...
          <div className="p-4 sm:p-5 rounded-2xl neu-inset bg-gradient-to-r from-emerald-50/40 via-white to-transparent border-l-4 border-emerald-600 space-y-1.5">
            <h4 className="font-black text-xs sm:text-sm text-slate-900 flex items-center gap-2">
              <ClipboardCheck className="w-4 h-4 text-emerald-700 shrink-0" />
              <span>Consider further clinical evaluation</span>
            </h4>
            <p className="text-xs font-medium text-slate-700 leading-relaxed">
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        {/* Main Section Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3 pb-2 border-b border-slate-300/60">
          <div>
            <h3 className="text-xl sm:text-3xl tracking-tight text-slate-900 font-black flex items-center gap-2.5 sm:gap-3">
              <span className="w-2.5 sm:w-3.5 h-6 sm:h-8 bg-gradient-to-b from-[#17805d] to-teal-700 rounded-full inline-block shrink-0"></span>
          ...
          {(() => {
            ...
            return sections.map((section) => {
              ...
              return (
                <div 
                  key={section.id} 
                  className={`p-4 sm:p-6 md:p-8 neu-flat rounded-3xl border border-slate-200/80 bg-white/90 shadow-[6px_6px_20px_rgba(163,177,198,0.35),-6px_-6px_20px_rgba(255,255,255,0.9)] transition-all duration-300 ${section.theme.wrapper}`}
                >
                  {/* Card Header Bar */}
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3 mb-4 sm:mb-5 pb-3 sm:pb-4 border-b border-slate-200/60">
                    <div className="flex items-center gap-2.5 sm:gap-3">
                      <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-2xl flex items-center justify-center shrink-0 ${section.theme.iconBg}`}>
                        {isFindings && (
                    ...
                    <span className={`text-[10px] sm:text-[11px] font-black uppercase tracking-wider px-3 py-1 rounded-full shadow-sm self-start sm:self-auto shrink-0 ${section.theme.badge}`}>
                      {section.badge}
                    </span>
                  </div>
      ...
      {/* 8. PATIENT EXAMINATION VITALS */}
      {payload && (
        <div className="space-y-3 sm:space-y-4 pt-1">
          <div className="flex items-center justify-between pb-1 border-b border-slate-300/50">
            <h4 className="text-[11px] sm:text-xs font-black uppercase tracking-widest text-slate-700 flex items-center gap-2">
              <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-blue-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
              <span>Patient Examination Vitals</span>
            </h4>
            <span className="text-[9px] sm:text-[10px] font-black uppercase text-slate-500 bg-white/80 px-2 sm:px-2.5 py-0.5 rounded-full border border-slate-200/60 shadow-sm shrink-0">
              Baseline
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5">
            ...
            <div className="p-4 sm:p-5 rounded-2xl neu-flat bg-gradient-to-br from-blue-50/90 via-sky-50/40 to-white/90 border border-blue-200/80 hover:scale-[1.02] transition-all shadow-sm">
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-blue-600 text-white flex items-center justify-center shadow-[0_4px_12px_rgba(37,99,235,0.35)] shrink-0">
                  <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                </div>
                <span className="text-[9px] sm:text-[10px] font-black uppercase text-blue-800 bg-blue-100/90 px-2 sm:px-2.5 py-0.5 rounded-full">Blood Pressure</span>
              </div>
              <div className="text-[10px] sm:text-xs font-black text-blue-900 tracking-wider uppercase">Resting Arterial</div>
              <div className="font-black text-lg sm:text-xl text-slate-950 mt-1">{payload.ap_hi ? `${payload.ap_hi}/${payload.ap_lo || 80} mmHg` : '120/80 mmHg'}</div>
            </div>

            {/* Tile 2: BMI */}
            <div className="p-4 sm:p-5 rounded-2xl neu-flat bg-gradient-to-br from-emerald-50/90 via-teal-50/40 to-white/90 border border-emerald-200/80 hover:scale-[1.02] transition-all shadow-sm">
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-emerald-600 text-white flex items-center justify-center shadow-[0_4px_12px_rgba(16,185,129,0.35)] shrink-0">
                ...
                <span className="text-[9px] sm:text-[10px] font-black uppercase text-emerald-800 bg-emerald-100/90 px-2 sm:px-2.5 py-0.5 rounded-full">Body Metrics</span>
              </div>
              <div className="text-[10px] sm:text-xs font-black text-emerald-900 tracking-wider uppercase">Body Mass Index</div>
              <div className="font-black text-lg sm:text-xl text-slate-950 mt-1">
            ...
            <div className="p-4 sm:p-5 rounded-2xl neu-flat bg-gradient-to-br from-amber-50/90 via-orange-50/40 to-white/90 border border-amber-200/80 hover:scale-[1.02] transition-all shadow-sm">
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-amber-500 text-white flex items-center justify-center shadow-[0_4px_12px_rgba(245,158,11,0.35)] shrink-0">
                  <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z" /></svg>
                </div>
                <span className="text-[9px] sm:text-[10px] font-black uppercase text-amber-800 bg-amber-100/90 px-2 sm:px-2.5 py-0.5 rounded-full">Biomarkers</span>
              </div>
              <div className="text-[10px] sm:text-xs font-black text-amber-900 tracking-wider uppercase">Cholesterol &bull; Glucose</div>
              <div className="font-black text-lg sm:text-xl text-slate-950 mt-1">
            ...
            <div className="p-4 sm:p-5 rounded-2xl neu-flat bg-gradient-to-br from-purple-50/90 via-violet-50/40 to-white/90 border border-purple-200/80 hover:scale-[1.02] transition-all shadow-sm">
              <div className="flex items-center justify-between mb-2 sm:mb-3">
                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-purple-600 text-white flex items-center justify-center shadow-[0_4px_12px_rgba(147,51,234,0.35)] shrink-0">
                ...
                <span className="text-[9px] sm:text-[10px] font-black uppercase text-purple-800 bg-purple-100/90 px-2 sm:px-2.5 py-0.5 rounded-full">Behavioral</span>
              </div>
              <div className="text-[10px] sm:text-xs font-black text-purple-900 tracking-wider uppercase">Lifestyle Habits</div>
              <div className="font-black text-lg sm:text-xl text-slate-950 mt-1">
